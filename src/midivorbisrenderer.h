...
namespace midirenderer
{
	...
	private:
		...
		void renderShortLoop(deleter_unique_ptr<fluid_synth_t>& synth, int& synthBufferIndex, int synthBufferSize,
		...
		void renderDoubleLoop(deleter_unique_ptr<fluid_synth_t>& synth, int& synthBufferIndex, int synthBufferSize,
		...
		void renderToBeatDivision(deleter_unique_ptr<fluid_synth_t>& synth, int& synthBufferPosition, int synthBufferSize, uint64_t& samplePosition, uint64_t lastTempoSample, int lastTempo, float* leftBuffer, float* rightBuffer, size_t& bufferIndex, OggVorbisEncoder& encoder);

		void readSampleFromSynth(deleter_unique_ptr<fluid_synth_t>& synth, int& synthBufferIndex, int synthBufferSize, float* leftBuffer, float* rightBuffer, size_t& bufferIndex, OggVorbisEncoder& encoder);
		void clearSynthBuffer(deleter_unique_ptr<fluid_synth_t>& synth, int& bufferPosition);
		...
		static void clearAndDeleteSynth(fluid_synth_t* synth);
